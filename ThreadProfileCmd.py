...
class _ThreadProfile(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyEnumeration","Angle","ThreadProfile",QT_TRANSLATE_NOOP("App::Property","Standard 60 degree V threads or experimental 3D printer-friendly 45 degree threads\n(Presets are invalid for 45 degree threads")).Angle=["60","45"]
...
"Also, the tolerances might be different from what you wish to have.  I believe the internal minor diameters are all minimum and the external are all maximum.",\
        ...
        obj.setEditorMode("Angle",2) #only shown for v thread types
    ...
    def makePoints(self, obj):
        ...
        if hasattr(obj,"Angle") and obj.Angle == "45": #only valid for v thread types
            if external:
...
class ThreadProfileCreateObjectCommandClass(object):
    ...
    def makeThreadProfile(self,name="VThreadProfile",minor_diameter=4.773,pitch=1,internal_or_external="External",internal_data=[],external_data=[],internal45_data=[],external45_data=[],presets=[],thread_count=10,Quality=11):
    ...
    that can be swept along a helix to produce a thread.  Code is based on Draft.makeBSpline()'''
        ...
        if len(internal45_data)==0:
            ...
            obj.internal45_data = internal_data
        if len(external45_data)==0:
            ...
            obj.external45_data = external_data
